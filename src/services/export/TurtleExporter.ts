/**
 * SQLite -> Turtle serialiser (the "Full Turtle" profile in ARCHITECTURE.md).
 *
 ...
 *
 * Pure database-to-string: used by the web part's "Export Turtle" command and
 * by tools/export-ttl.ts (CLI), so the smoke test exercises exactly the code
 * that ships.
 */
...
export interface ITurtleExportStats {
  classes: number;
  properties: number;
  concepts: number;
  labels: number;
  relationshipTriples: number;
  passthrough: number;
}
...
export function exportTurtle(db: Database): { ttl: string; stats: ITurtleExportStats } {
  ...
  return {
    ...
    stats: {
      ...
      labels: labelCount,
      relationshipTriples: relTriples,
